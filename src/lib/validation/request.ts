...
/** Manager-initiated היעדרות (absence) for a worker, from the schedule requests
 *  view. `kind` distinguishes ordinary vacation, military reserve duty
 *  (מילואים) and sick leave (מחלה) — all are hard-off ranges for the
 *  scheduler, only the label differs. */
export const managerAddVacationSchema = z
  .object({
    employeeId: z.string().uuid({ message: 'מזהה עובד לא תקין' }),
    dateFrom: z.string().regex(isoDateRegex, { message: 'תאריך התחלה לא תקין' }),
    dateTo: z.string().regex(isoDateRegex, { message: 'תאריך סיום לא תקין' }),
    kind: z.enum(['vacation', 'miluim', 'sick']).default('vacation'),
  })
  .refine((d) => d.dateTo >= d.dateFrom, {
