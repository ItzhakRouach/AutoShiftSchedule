...
test('manager sets a vacation and a מילואים range for workers from the requests view', async ({ page }) => {
  ...
  // ── 1. Open דנה כהן's vacation sheet and add an ordinary vacation range ────
  const danaRow = page.locator('tr').filter({ hasText: 'דנה כהן' })
  await danaRow.getByRole('button', { name: 'חופשה' }).click()

  await expect(page.getByRole('heading', { name: /חופשה — דנה כהן/ })).toBeVisible({ timeout: 8000 })
  ...
  await expect(page.getByRole('heading', { name: /חופשה — דנה כהן/ })).toBeHidden({ timeout: 5000 })

  // ── 2. Open יוסי לוי's sheet and add a מילואים range ───────────────────────
  const yossiRow = page.locator('tr').filter({ hasText: 'יוסי לוי' })
  await yossiRow.getByRole('button', { name: 'חופשה' }).click()
  await expect(page.getByRole('heading', { name: /חופשה — יוסי לוי/ })).toBeVisible({ timeout: 8000 })
  ...
  await expect(page.getByText('אין חופשות מוגדרות')).toBeVisible({ timeout: 10000 })
})
